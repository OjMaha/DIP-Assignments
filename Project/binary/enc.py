...
def encoder(image_path, quality_factor, output_file, Q):
    ...
    for diff in DC_differences:
        ...
        dc_encoded_data.extend(huffman_code)
    ...
    for i in range(0, len(AC_coefficients), 63):
        ...
        for code, count in encoded_block:
            if code == -1:  # End of block marker
                ac_encoded_data.extend("11111111001")  # Example EOB code
            else:
                ac_encoded_data.extend(code) # Variable-length code
                ac_encoded_data.extend(f"{count:04b}")  # 4-bit run length encoding

    # Save data to a binary file
    with open(output_file, 'wb') as f:
        # Write metadata: Original dimensions and quality factor
        f.write(struct.pack("HH", original_height, original_width))  # 2 bytes each
        f.write(struct.pack("H", quality_factor))  # 2 bytes for quality factor
        
        # Write DC Huffman codes
        for symbol, code in dc_huffman_codes.items():
            code_bits = bitarray(code)
            f.write(struct.pack("b", symbol))  # 1 byte for the symbol
            f.write(struct.pack("B", len(code)))  # 1 byte for the code length
            f.write(code_bits.tobytes())  # Variable-length code

        #header end marker for DC codes
        f.write(struct.pack("b", -128))

        # Write AC Huffman codes
        ...
        #header end marker for AC codes
        f.write(struct.pack("b", -128))

        # Write encoded DC data
        f.write(dc_encoded_data.tobytes())

        #end of DC data
        f.write(struct.pack("b", -128))
        
        # Write encoded AC data
        f.write(ac_encoded_data.tobytes())

    print(f"Encoded data and metadata saved to {output_file} as a binary file.")
