...
def upsample(channel, original_size):
    """Upsample back to original dimensions."""
    return channel.repeat(2, axis=0).repeat(2, axis=1)[:original_size[0], :original_size[1]]
...
def decode_image(file_Y, file_Cb, file_Cr, quality_factor, Q):
    # Decode Y, Cb, and Cr channels
    Y = decoder(file_Y, quality_factor, Q)
    Cb = decoder(file_Cb, quality_factor, Q)
    Cr = decoder(file_Cr, quality_factor, Q)
    ...
    Cb = upsample(Cb, original_size)
    Cr = upsample(Cr, original_size)
    ...
    decompressed_img = combine_rgb(R, G, B)
